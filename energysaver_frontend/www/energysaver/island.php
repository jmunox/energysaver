...
if(!isset( $_SESSION['auth'] )){//If session not registered
	...
	if(preg_match('/MSIE/i',$user_agent)||preg_match('/Internet Explorer/i',$user_agent)){
		include("island_ie.php");
	}else{
     ...
     function init() {  
		...
		//ctx.scale(scalefactorX, scalefactorY);
			...
			ctx.drawImage(usr4,  
				...
				ctx.font = "bold 13px Verdana"; // Change the size and font
	    ...
	    if (imgW <= CanvasXSize) {  
	        ...
	        if (x > (CanvasXSize-imgW)) { ctx.drawImage(water,  
	        		0,0,1000,300,x-CanvasXSize+1,water4_posy,1000,300); }  
	    }  
	    //If image is > Canvas Size  
	    else {  
	        //reset, start from beginning  
	        ...
	        //draw aditional image  
	        if (x > (CanvasXSize-imgW)){ ctx.drawImage(water,  
	        		0,0,1000,300,x-imgW+1,water4_posy,1000,300); }  
	    }  
	    //draw image  
	    ctx.drawImage(water,  
	    		0,0,1000,300,x,water4_posy,1000,300); 
	    //amount to move  
	    x += dx;  

	    ctx.drawImage(scaleImg,  
	    		0,0,40,240,scaleImg_posx,scaleImg_posy,40,240); 
